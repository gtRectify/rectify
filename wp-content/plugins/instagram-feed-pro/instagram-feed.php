...
if (function_exists('sb_instagram_feed_init') || function_exists('display_instagram')) {
	...
	function sb_instagram_feed_init()
	{
		...
		if (class_exists('\InstagramFeed\Vendor\Smashballoon\Framework\Packages\Feedback\FeedbackManager')) {
			\InstagramFeed\Vendor\Smashballoon\Framework\Packages\Feedback\FeedbackManager::init([
				'plugin_slug'        => 'instagram-feed-pro',
				'plugin_name'        => 'Smash Balloon Instagram Feed Pro',
				'plugin_version'     => SBIVER,
				'plugin_file'        => __FILE__,
				'support_url'        => 'https://smashballoon.com/support/?utm_campaign=instagram-pro&utm_source=support&utm_medium=support',
		...
		require_once trailingslashit(SBI_PLUGIN_DIR) . 'inc/class-sb-instagram-oembed-metadata.php';
		...
		$sbi_smash_usage_tracking = new \InstagramFeed\UsageTracking\SmashUsageTracking();
	...
	function sb_instagram_uninstall()
	{
		...
		// Usage-tracking consent, site token and telemetry are shared with the free plugin. Only remove
		// them when the free plugin is not active, otherwise deleting Pro would reset the free plugin's consent.
		$sbi_network_active = is_multisite() ? (array) get_site_option( 'active_sitewide_plugins', array() ) : array();
		$sbi_free_active    = in_array( 'instagram-feed/instagram-feed.php', (array) get_option( 'active_plugins', array() ), true )
			|| isset( $sbi_network_active['instagram-feed/instagram-feed.php'] );
		if ( ! $sbi_free_active ) {
		...
		wp_clear_scheduled_hook( 'sbi_smash_usage_tracking_cron' );

		delete_option('sb_instagram_feed_notices');
		delete_option('sb_instagram_feed_group_notices');

		// Clear og:image metadata cache (for thumbnail_url deprecation solution)
		if (class_exists('SB_Instagram_OEmbed_Metadata')) {
			SB_Instagram_OEmbed_Metadata::clear_all_cache();
		}
	}
