		...
		var a = jQuery,
			...
			c = function() {
				...
				return b.prototype.init = function() {
					...
					}), this.$lightbox.find(".sbi_lb-container").hammer().on("swiperight", function(e) {
						// SMASH-863: Don't navigate to prev post if swiping inside an active carousel
						var swipeTarget = (e.gesture && e.gesture.target) ? e.gesture.target : e.target;
						if (swipeTarget && jQuery(swipeTarget).closest('.sbi_lb-image-wrap.sbi-owl-loaded').length) {
							return !1;
						}
						sbLbCarouselDestroy();
						if( sbi_supports_video() && jQuery('#sbi_lightbox video.sbi_video').length ) jQuery('#sbi_lightbox video.sbi_video')[0].pause();
					...
					}), this.$lightbox.find(".sbi_lb-container").hammer().on("swipeleft", function(e) {
						// SMASH-863: Don't navigate to next post if swiping inside an active carousel
						var swipeTarget = (e.gesture && e.gesture.target) ? e.gesture.target : e.target;
						if (swipeTarget && jQuery(swipeTarget).closest('.sbi_lb-image-wrap.sbi-owl-loaded').length) {
							return !1;
						}
					...
					if (this.album[this.currentImageIndex].video === 'link') {
						var gdprNoticeUrl = this.album[this.currentImageIndex].url;
						var $gdprNotice = jQuery(".sbi_lb-container-wrapper .sbi_gdpr_notice");
						if (!$gdprNotice.length) {
							jQuery(".sbi_lb-container-wrapper").prepend('<a href="'+gdprNoticeUrl+'" target="_blank" rel="noopener noreferrer" class="sbi_gdpr_notice"><svg style="color: rgba(255,255,255,1)" class="svg-inline--fa fa-play fa-w-14 sbi_playbtn" aria-label="Play" aria-hidden="true" data-fa-processed="" data-prefix="fa" data-icon="play" role="presentation" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path fill="currentColor" d="M424.4 214.7L72.4 6.6C43.8-10.3 0 6.1 0 47.9V464c0 37.5 40.7 60.1 72.4 41.3l352-208c31.4-18.5 31.5-64.1 0-82.6z"></path></svg></a>');
						} else {
							$gdprNotice.attr('href', gdprNoticeUrl);
						}
...
//                  console.log('not raising res for replacement  ' + currentUrl, reason);
		...
		function SbiFeedPro(el, index, settings) {
			...
			this.initLayout = function () {
				...
				if ( layout === 'carousel' ) {
					$sbi.find('#sbi_images').addClass('sbi_carousel').attr('role','region').attr('aria-label','Instagram Feed Carousel');
