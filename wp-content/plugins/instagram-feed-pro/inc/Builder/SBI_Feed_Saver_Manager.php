...
class SBI_Feed_Saver_Manager
{
	...
	public static function feed_customizer_fly_preview()
	{
		...
		if (isset($_POST['feedID']) && isset($_POST['previewSettings'])) {
			...
			// SMASH-1044: For mixed feeds with moderation enabled, clearing the cache on every
			// customizer interaction triggers fresh API calls for ALL sources (user timeline +
			// tagged posts), causing severe slowness in WP admin. Skip the cache clear when
			// the moderation mode setting is active — the existing cache is perfectly adequate
			// for showing a settings preview, and will be rebuilt on the next frontend page load.
			$moderation_mode_enabled = !empty($preview_settings['enablemoderationmode'])
				&& $preview_settings['enablemoderationmode'] !== 'false'
				&& $preview_settings['enablemoderationmode'] !== false;

			if (!$moderation_mode_enabled) {
				$feed_cache = new SB_Instagram_Cache($feed_id);
				$feed_cache->clear('all');
				$feed_cache->clear('posts');
			}
	...
	public static function moderation_pagination_feed($opt, $preview_settings)
	{
		...
		if ($settings['caching_type'] === 'permanent' && empty($settings['doingModerationMode'])) {
			...
			// SMASH-1044: Ensure allow-list (moderation-queue) posts are added from DB
			// even when no cache exists. Previously this only happened in the "cache exists"
			// branch, so a cache-clear caused queued posts to disappear.
			if ($instagram_feed->using_an_allow_list($settings)) {
				$instagram_feed->add_report('Adding allow list only posts (no-cache path)');
				$instagram_feed->add_db_only_allow_list_posts($settings);
			}
		}
