...
class SBI_Source
{
	...
	public static function builder_update_multiple()
	{
		...
		if (!empty($_POST['sourcesList']) && is_array($_POST['sourcesList'])) {
	 ...
	 *
	 * @param array $source_data Source data to process.
	 *
	 * @return string
	 */
	public static function process_connecting_source_data($source_data)
	{
		...
		if (!empty($single_source['error'])) {
			...
			if (isset($single_source['error']['response']) && is_wp_error($single_source['error']['response'])) {
				...
				/* translators: %s is the URL that failed to connect */
			...
			$message .= ' <a href="https://smashballoon.com/instagram-feed/docs/errors/?utm_campaign=instagram-pro&utm_source=customizer&utm_medium=docs" target="_blank" rel="noopener">' . __('Directions on how to resolve this issue', 'instagram-feed') . '</a>';
	 ...
	 *
	 * @param array $connected_account Connected account array.
	 * @param bool  $connect_if_error Whether or not to reconnect if error.
	 *
	 * @return array
	 *
	 * @since 6.0
	...
	/**
	 * Merges account details from the header details array with the connected account.
	 *
	 * @param array $header_details_array An array containing header details.
	 * @param array $connected_account The connected account object.
	 *
	 * @return array The merged account details.
	 */
	 ...
	 *
	 * @param array $source_data Source data to update or insert.
	 *
	 * @return bool
	 *
	 * @since 6.0
	 ...
	 *
	 * @param array $args Args to check if they exists in database.
	 *
	 * @return bool
	 * @since 6.0
	 */
	 ...
	 *
	 * @param array $source_data Source data to update.
	 * @param bool  $where_privilige Whether or not to check the where privilege.
	 *
	 * @return false|int
	 * @since 6.0
	 */
	 ...
	 *
	 * @param array $source_data Source data to insert.
	 *
	 * @return false|int
	 * @since 6.0
	 */
	 ...
	 *
	 * @param bool $is_settings Check if the given page is settings page.
	 *
	 * @return array
	 * @since 6.0
	 */
		...
		if (empty($pages_data_arr['data'])) {
			return array(
				'error' => array(
					...
					/* translators: %1$s and %2$s are HTML anchor tags for the Facebook account link */
	...
	public static function should_do_source_updates()
	{
		...
		$should_do_source_updates = !empty($sbi_statuses_option['legacy_source_queue']);
	 ...
	 * the update is done.
	 *
	 * @param string $slug_or_id Slug or id to get the feed id.
	 *
	 * @return array|bool
	 */
	 ...
	 * account
	 *
	 * @param string $account_id The account id to clear the error logs.
	 *
	 * @return bool
	 *
	 * @since 6.0
	 ...
	 *
	 * @param string              $account_id Account id that occured the error.
	 * @param string|object|array $error Database Error .
	 *
	 * @return bool
	 *
	 * @since 6.0
	 ...
	 *
	 * @param array $source_data Source id to convert to connected accounts.
	 *
	 * @return array
	 * @since 6.0
	 */
	 ...
	 *
	 * @param string $account_id Account id to update bio.
	 * @param string $bio Bio for the account id.
	 *
	 * @return void
	 *
	 * @since 6.1
