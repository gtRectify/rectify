...
class SB_Instagram_Posts_Manager
{
	 ...
	 * @var mixed|void
	 */
	protected $sbi_options;

	/**
	 * Limit of how many records can be stored in the database.
	 *
	 * @var int
	 */
	protected $limit;
	...
	/**
	 * Errors that have been encountered, stored in the database.
	 *
	 * @var array
	 */
	protected $errors;
	...
	/**
	 * Errors that are displayed to users.
	 *
	 * @var array
	 */
	protected $display_error;

	/**
	 * Whether or not the tables for resizing images exist.
	 *
	 * @var bool
	 */
	protected $resizing_tables_exist;
	 ...
	 * was made
	 *
	 * @param string $hashtag The hashtag to check for in the list of top API calls.
	 *
	 * @return bool
	 */
	 ...
	 *
	 * @param string $hashtag The hashtag to remove from the list of top API calls.
	 */
	...
	/**
	 * Add a hashtag to the list of top API calls already made.
	 *
	 * @param string $hashtag The hashtag to add to the list of top API calls.
	 */
	 ...
	 *
	 * @param array              $connected_account Connected account data.
	 * @param string             $error_type The type of error.
	 * @param mixed/array/string $details    The details of the error.
	 *
	 * @since 2.7/5.10
	 */
	 ...
	 * @param array|string     $details The details of the error.
	 * @param mixed/bool/array $connected_account_term The connected account that encountered the error.
	 *
	 * @since 2.7/5.10
	 */
	 ...
	 *
	 * @param array $details The details of the error.
	 *
	 * @return bool
	 * @since 2.7/5.10
	 */
	...
	public function generate_error_message($response, $connected_account = array('username' => ''))
	{
		...
		if (isset($response['response']) && is_wp_error($response['response'])) {
			...
			/* translators: the Instagram API url */
			$error_message_return['admin_only'] = sprintf(__('Error connecting to %s.', 'instagram-feed'), $response['url']);

			$error_message_return['frontend_directions'] = '<a href="https://smashballoon.com/instagram-feed/docs/errors/?utm_campaign=instagram-pro&utm_source=error-message&utm_medium=docs' . $hash . '" target="_blank" rel="noopener">' . __('Directions on how to resolve this issue', 'instagram-feed') . '</a>';
		...
		if (isset($response['error']['message'])) {
			if ((int)$response['error']['code'] === 100) {
				...
				$error_message_return['frontend_directions'] = '<a href="https://smashballoon.com/instagram-feed/docs/errors/?utm_campaign=instagram-pro&utm_source=error-message&utm_medium=docs" target="_blank" rel="noopener">' . __('Directions on how to resolve this issue', 'instagram-feed') . '</a>';
			} else {
				if ((int)$response['error']['code'] === 999) {
					$url = 'https://smashballoon.com/doc/error-999-access-token-could-not-be-decrypted/?utm_campaign=instagram-pro&utm_source=error-message&utm_medium=docs';
				} else {
					$url = 'https://smashballoon.com/instagram-feed/docs/errors/?utm_campaign=instagram-pro&utm_source=error-message&utm_medium=docs' . $hash;
				}
	...
	/**
	 * Checks if the plugin is in a state where it should not resize images
	 *
	 * @param bool $data The data to check.
	 *
	 * @return bool
	 * @since 2.0/4.0
	 */
	public function image_resizing_disabled($data = false)
	{
		...
		$disable_resizing = isset($options['sb_instagram_disable_resize']) && ($options['sb_instagram_disable_resize'] === 'on' || $options['sb_instagram_disable_resize'] === true);
	...
	/**
	 * Removes an error from the database
	 *
	 * @param string           $type The type of error.
	 * @param mixed/array/bool $connected_account The connected account that encountered the error.
	 *
	 * @since 2.0/4.0
	 */
	 ...
	 *
	 * @param string $log_item The log item to add.
	 *
	 * @since 2.7/5.10
	 */
	...
	/**
	 * Removes an error related to a connected account
	 *
	 * @param array  $clearing_account The connected account that encountered the error.
	 * @param string $clearing_error_type The type of error.
	 * @param bool   $update Whether or not to update the option.
	 *
	 * @return bool
	 *
	 * @since 2.7/5.10
	...
	/**
	 * Checks if an account reached the hashtag limit.
	 *
	 * @param array $account The connected account that encountered the error.
	 *
	 * @return bool
	 * @since 2.7/5.10
	 */
	...
	/**
	 * Checks if an account has a critical error
	 *
	 * @param array $connected_account The connected account that encountered the error.
	 *
	 * @return bool
	...
	public function hashtag_has_error($hashtag)
	{
		...
		foreach ($this->errors['hashtag'] as $hashtag_error) {
			...
			} elseif (!empty($hashtag_error['clear_time'])) {
				if ($hashtag_error['clear_time'] < time()) {
					$changed = true;
				} else {
					$to_save[] = $hashtag_error;
		...
		if (isset($this->errors['connection']['critical'])) {
			...
			if ($errors['connection']['error_id'] === 190) {
				...
				if (!empty($error_message_array['backend_directions'])) {
					...
					$error_message .= '<div class="license-action-btns"><p class="sbi-error-directions"><a class="sbi-license-btn sbi-btn-blue sbi-notice-btn" href="https://smashballoon.com/instagram-feed/docs/errors/?utm_campaign=instagram-pro&utm_source=error-message&utm_medium=docs' . $hash . '" target="_blank" rel="noopener">' . __('Directions on how to resolve this issue', 'instagram-feed') . '</a>' . $retry . '</p></div>';
				}
			}
		} else {
			$connected_accounts = SB_Instagram_Connected_Account::get_all_connected_accounts();
			foreach ($connected_accounts as $connected_account) {
				if (
					...
					$error_message_array = array(
						...
						'frontend_directions' => '<a href="https://smashballoon.com/instagram-feed/docs/errors/?utm_campaign=instagram-pro&utm_source=error-message&utm_medium=docs#10">' . __('Click here to troubleshoot', 'instagram-feed') . '</a>',
						'backend_directions' => '',
					);
				}

				if (!empty($this->errors['accounts'][$connected_account['user_id']]['api']['error']) && $this->is_critical_error($this->errors['accounts'][$connected_account['user_id']]['api'])) {
				...
				if (!isset($error_message) && isset($error_message_array)) {
					...
					if (!empty($error_message_array['backend_directions'])) {
						...
						if (!empty($this->errors['connection']['error_id']) && $this->errors['connection']['error_id'] === 999) {
							...
							$url = 'https://smashballoon.com/instagram-feed/docs/errors/?utm_campaign=instagram-pro&utm_source=error-message&utm_medium=docs';
						}
	...
	/**
	 * Get the error id for the app permission related error
	 *
	 * @return array
	 */
	...
	/**
	 * Check if the current API request is delayed
	 *
	 * @param array $connected_account The connected account that encountered the error.
	 *
	 * @return bool
