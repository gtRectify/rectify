...
class SBI_New_User extends SBI_Notifications
{
	 ...
	 * @var string
	 */
	public const SOURCE_URL = 'https://plugin.smashballoon.com/newuser.json';
	 ...
	 * @var string
	 */
	public const OPTION_NAME = 'sbi_newuser_notifications';
	...
	/**
	 * Retrieve the option name for the Instagram Feed Pro plugin.
	 *
	...
	/**
	 * Display a review notice consent message to the user.
	 *
	 * @return void
	 */
	...
	public function output()
	{
		...
		foreach ($notifications as $notification) {
			...
			switch ($type) {
				case 'review':
					$sbi_open_feedback_url = 'https://smashballoon.com/feedback/?plugin=instagram-pro&utm_campaign=instagram-pro&utm_source=new-user&utm_medium=feedback';
	...
	public function verify_active($notifications)
	{
		...
		$has_been_one_month_since_rating_dismissal = !isset($sbi_statuses_option['rating_notice_dismissed']) || ((int)$sbi_statuses_option['rating_notice_dismissed'] + ((int)$notifications['review']['wait'] * DAY_IN_SECONDS)) < $current_time + 1;
	 ...
	 *
	 * @param array $notification A single notification.
	 *
	 * @return string $title
	 * @since 6.0
	 */
	 ...
	 * @param array $content_allowed_tags Content allowed tags.
	 *
	...
	public function get_notice_content($notification, $content_allowed_tags)
	{
		...
		} elseif (!empty($notification['content'])) {
