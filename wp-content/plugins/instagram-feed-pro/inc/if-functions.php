...
function display_instagram($atts = array(), $preview_settings = false)
{
	...
	if ($settings['caching_type'] === 'permanent' && empty($settings['doingModerationMode'])) {
		...
		// SMASH-1044: Also pull allow-list posts from DB when building a fresh cache (no prior cache).
		// Previously add_db_only_allow_list_posts() was only called in the "cache exists" branch,
		// so clearing the cache caused moderation-queue posts to vanish until a second page load.
...
function sbi_get_feed_template_part($template_part, $settings = array())
{
	...
	$cache_key = sanitize_key(implode('-', ['template', $template_name, $custom_templates_enabled ? 'custom' : 'default']));
