...
{
	/**
	 * Next pages of data available for each feed term.
	 *
	 * @var array
	 */
	...
	/**
	 * Cache object used to store and retrieve data.
	 *
	...
	/**
	 * Posts found in the API request.
	 *
	 * @var array
	 *
	 * @since 2.1.3/5.2.3
	...
	/**
	 * Encryption object used to encrypt and decrypt data.
	 *
	...
	/**
	 * Transient name used to store the feed data.
	 *
	 * @var string
	 */
	...
	/**
	 * Transient name used to store the header data.
	 *
	 * @var string
	 */
	...
	/**
	 * Transient name used to store the backup feed data.
	 *
	 * @var string
	 */
	...
	/**
	 * Transient name used to store the backup header data.
	 *
	 * @var string
	 */
	...
	/**
	 * Data retrieved from the API request.
	 *
	 * @var array
	 */
	private $post_data;
	...
	/**
	 * Header data retrieved from the API request.
	 *
	 * @var array
	 */
	private $header_data;
	...
	/**
	 * Transient attributes used to store the shortcode atts.
	 *
	 * @var array
	 */
	...
	/**
	 * Timestamp of the last retrieval from the API.
	 *
	 * @var int
	 */
	...
	/**
	 * Number of API calls made for this feed.
	 *
	 * @var int
	 */
	...
	/**
	 * Should the backup feed be used.
	 *
	 * @var bool
	 */
	...
	/**
	 * Resized images for this post set.
	 *
	 * @var array
	 *
	 * @since 2.1.1/5.2.1
	...
	public function __construct($transient_name)
	{
		...
		// count of how many api calls have been made for each feed type and term.
	 ...
	 * @param bool      $should_cache Should the data be cached.
	 *
	 * @return array
	 *
	 * @since 2.0/5.0
	 ...
	 * on the page.
	 *
	 * @param array $array_of_ids Instagram IDs of the posts to update.
	 *
	 * @since 2.0/5.0
	 */
	...
	/**
	 * Retrieves the feed ID by removing asterisks from the regular feed transient name.
	 *
	 * @return string The feed ID with asterisks removed.
	 */
	...
	/**
	 * Sets the cache for the Instagram feed.
	 *
	 * @param int         $cache_seconds The number of seconds to cache the feed.
	 * @param array       $settings The settings for the feed.
	 * @param bool|string $feed_id Optional. The ID of the feed. Default false.
	 *
	 * If no feed ID is provided, the regular feed transient name will be used.
	 */
	...
	/**
	 * Gets the post data for the feed.
	 *
	 * @return array The post data for the feed.
	 * @since 2.0/5.0
	 */
	...
	/**
	 * Sets the post data for the feed.
	 *
	 * @param array $post_data The post data for the feed.
	 * @since 2.0/5.0
	 */
	...
	/**
	 * Get the next pages of data available for each feed term.
	 *
	 * @return array
	 * @since 2.0/5.0
	 */
	...
	/**
	 * Set the number of pages created.
	 *
	 * @param int $num The number of pages to set.
	 */
	...
	/**
	 * Retrieves the header data for the feed.
	 *
	 * @since 2.0/5.0
	 */
	...
	/**
	 * Sets the header data for the feed.
	 *
	 ...
	 *
	 * @param array $atts Atts available for cron caching.
	 *
	 * @since 2.0/5.0
	 */
	 ...
	 *
	 * @param string $to_add String to add to the report.
	 *
	 * @since 2.0/5.0
	 */
	 ...
	 *
	 * @param int $num Number of posts needed.
	 * @param int $offset Number of posts already created.
	 * @param int $page Number of pages created.
	 *
	 * @return bool
	 * @since 2.0/5.0
	 */
	...
	public function add_remote_posts($settings, $feed_types_and_terms, $connected_accounts_for_feed)
	{
		 ...
		 *
		 * @param int $num        Minimum number of posts needed in each API request.
		 * @param array $settings Settings for this feed.
		 *
		 * @since 2.0/5.0
		 */
		...
		foreach ($feed_types_and_terms as $type => $terms) {
			...
			foreach ($terms as $term_and_params) {
				...
				if (
					...
					} elseif (!$connection->is_wp_error() && $this->can_try_another_request($type, $connected_accounts_for_feed[$term])) {
	 ...
	 * Overwritten in the Pro version
	 *
	 * @param array  $connected_account_or_page The connected account or page data.
	 * @param string $type The type of feed data to retrieve.
	...
	/**
	 * Clears the error for an account if the access token has expired.
	 *
	 * @param array $connected_account_for_term The connected account data.
	 *
	 * @since 2.0/5.1.2
	 */
	...
	{
		// array_unique( $post_set, SORT_REGULAR) removes duplicate posts.
		return $post_set;
	}

	/**
	 * Gets the account term for the post set
	 *
	 * @param array $term_and_params The term and parameters for the account.
	 * @return string The account term.
	 */
	 ...
	 *
	 * @param string $type The type of feed data to retrieve.
	 * @param array  $connected_account_with_error The connected account data.
	 ...
	 *
	 * @param string $type The type of feed data to retrieve.
	 * @param array  $attempted_connected_accounts The connected accounts that have been attempted.
	 *
	 * @return bool
	 * @since 2.0/5.1.1
	 */
	...
	public function set_remote_header_data($settings, $feed_types_and_terms, $connected_accounts_for_feed)
	{
		...
		if (!empty($settings['headersource'])) {
			foreach ($connected_accounts_for_feed as $connected_account) {
				if ($connected_account['username'] === $settings['headersource'] || $connected_account['user_id'] === $settings['headersource']) {
	 ...
	 * @param bool $save_backup Whether to also save this as a permanent cache.
	 * @param bool $force_cache Whether to force the cache to be saved.
	 *
	 * @since 2.0/5.0
	...
	/**
	 * Removes duplicate posts from the feed
	 *
	 * @since 2.0/5.1
	 */
	...
	/**
	 * Checks if there is an error to cache
	 *
	 * @return bool
	 * @since 2.0/5.0
	 */
	 ...
	 * @param array $connected_accounts_for_feed Connected account data for the
	 *  feed types and terms.
	 * @param bool  $moderation_posts Posts to display if in moderation mode.
	 *
	 * @return false|string
	 *
	 * @since 2.0/5.0
	...
	public function get_the_feed_html($settings, $atts, $feed_types_and_terms, $connected_accounts_for_feed, $moderation_posts = false)
	{
		...
		} elseif (isset($feed_types_and_terms['users']) && isset($this->post_data[0])) {
			// in case no connected account and no header.
	...
	/**
	 * Generates the HTML for the feed when no posts are found
	 *
	 * @param array $settings Settings for this feed.
	 * @param array $feed_types_and_terms Organized settings related to feed data
	 *  (ex. 'user' => array( 'smashballoon', 'custominstagramfeed' ).
	...
	protected function handle_no_posts_found($settings = array(), $feed_types_and_terms = array())
	{
		...
		$error_message_return = array(
			...
			'frontend_directions' => '<a href="https://smashballoon.com/instagram-feed/docs/errors/?utm_campaign=instagram-pro&utm_source=error-message&utm_medium=docs">' . __('Click here to troubleshoot', 'instagram-feed') . '</a>',
			'backend_directions' => '<a href="https://smashballoon.com/instagram-feed/docs/errors/?utm_campaign=instagram-pro&utm_source=error-message&utm_medium=docs">' . __('Click here to troubleshoot', 'instagram-feed') . '</a>'
		);
	 ...
	 *
	 * @param array $settings Settings for this feed.
	 * @param int   $offset The number of posts already displayed.
	 *
	 * @return bool
	 * @since 2.0/5.0
	 */
	public function should_use_pagination($settings, $offset = 0)
	{
		...
		if ($show_loadmore_button_by_settings) {
			...
			// SMASH-1044: Only check API cursor when the current page is fully populated.
			// Heavy moderation (block_list on a large mixed feed) can leave posts_available < 0
			// even though the API returned a next-page cursor, causing a Load More button that
			// yields zero new posts when clicked.
			if ( $posts_available >= 0 ) {
	...
	/**
	 * Determines if the backup cache should be used
	 *
	 * @return bool
	 * @since 2.0/5.0
	 */
	 ...
	 * Overwritten in the Pro version
	 *
	 * @param string $other_atts Additional attributes for the feed container.
	 * @param array  $settings Settings for this feed.
	 *
	 * @return string
	 */
	...
	/**
	 * Get the resized images for the feed
	 *
	 * @return array
	 * @since 2.1.1/5.2.1
	 */
	...
	/**
	 * Set the resized images for the feed
	 *
	 * @param array $resized_image_data Resized image data for the feed.
	 * @since 2.1.1/5.2.1
	 */
