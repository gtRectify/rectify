		...
		foreach ($this->connected_accounts_in_feed as $connected_account_in_feed) {
			if (
				...
				$error_message_return = array(
					...
					'frontend_directions' => '<a href="https://smashballoon.com/instagram-feed/docs/errors/?utm_campaign=instagram-pro&utm_source=settings&utm_medium=docs#10">' . __('Click here to troubleshoot', 'instagram-feed') . '</a>',
					'backend_directions' => ''
				);
	...
	private function set_hashtag_feed($hashtags)
	{
		...
		if (empty($connected_business_accounts)) {
			...
			$error_message_return = array(
				...
				'admin_only' => sprintf(__('A business account is required to display hashtag feeds. Please visit %s to learn how to connect a business account.', 'instagram-feed'), '<a href="https://smashballoon.com/migrate-to-new-instagram-hashtag-api/?utm_campaign=instagram-pro&utm_source=settings&utm_medium=docs">' . __('this page', 'instagram-feed') . '</a>'),
				'frontend_directions' => '',
				'backend_directions' => ''
			);
			$sb_instagram_posts_manager->maybe_set_display_error('configuration', $error_message_return);
