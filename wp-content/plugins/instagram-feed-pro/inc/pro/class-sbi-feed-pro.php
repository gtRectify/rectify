...
class SB_Instagram_Feed_Pro extends SB_Instagram_Feed
{
	...
	public function filter_posts($post_set, $settings = array())
	{
		...
		// before anything else — must run ahead of the no-user-filters early return below,
		// otherwise feeds with default settings would still show trial reels.
	...
	protected function handle_no_posts_found($settings = array(), $feed_types_and_terms = array())
	{
		...
		if (
			...
			if (!empty($settings['whitelist'])) {
				$error_message_return['admin_only'] = __('Use <a href="https://smashballoon.com/guide-to-moderation-mode/?utm_campaign=instagram-pro&utm_source=error-message&utm_medium=docs">moderation mode</a> to add posts to your whitelist.', 'instagram-feed');
			} else {
				$error_message_return['admin_only'] = __('You may be filtering out too many posts or there could be a cache conflict. See <a href="https://smashballoon.com/post-filtering-not-working/?utm_campaign=instagram-pro&utm_source=error-message&utm_medium=docs">this page</a> for troubleshooting steps.', 'instagram-feed');
			}
