...
class Platform_Data
{
	 ...
	 * @var string
	 */
	protected const SBI_STATUSES_OPTION_KEY = 'sbi_statuses';
	 ...
	 * @var string
	 */
	protected const REVOKE_PLATFORM_DATA_OPTION_KEY = 'sbi_revoke_platform_data';
	 ...
	 * @var string
	 */
	protected const APP_PERMISSION_REVOKED_STATUS_KEY = 'app_permission_revoked';

	/**
	 * Array key for the warning email flag for unused feed status key on `sbi_statuses`.
	 */
	protected const UNUSED_FEED_WARNING_EMAIL_SENT_STATUS_KEY = 'unused_feed_warning_email_sent';
	 ...
	 *
	 * @param array $to_delete_connected_account The connected account to delete.
	 *
	 * @return void
	 */
	...
	protected function send_platform_data_delete_notification_email()
	{
		...
		/* translators: 1. Home url 2. Website name */
		$site_url = sprintf('<a href="%1$s">%2$s<a/>', esc_url(home_url()), __('your website', 'instagram-feed'));
		...
		/* translators: 1. Link to Settings Page 2. Settings Page */
		$settings_page = sprintf('<a href="%1$s">%2$s</a>', esc_url($link), esc_html__('Settings Page', 'instagram-feed'));
	...
	protected function send_revoke_notification_email()
	{
		...
		/* translators: 1. Link to Settings Page 2. Settings Page */
		$settings_page = sprintf('<a href="%1$s">%2$s</a>', esc_url($link), esc_html__('Settings Page', 'instagram-feed'));
		/* translators: Settings page */
		$details .= '<p>' . sprintf(__('To prevent the automated deletion of data for the source, please reconnect your source for the Instagram Feed plugin %s within 7 days.', 'instagram-feed'), $settings_page) . '</p>';
		$details .= '<p><a href="https://smashballoon.com/doc/action-required-within-7-days/?instagram&utm_campaign=instagram-pro&utm_source=permissionerror&utm_medium=email&utm_content=More Information" target="_blank" rel="noopener">' . __('More Information', 'instagram-feed') . '</a></p>';
