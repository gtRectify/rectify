...
use InstagramFeed\Vendor\DI\Annotation\Inject;
use InstagramFeed\Vendor\DI\Annotation\Injectable;
use InstagramFeed\Vendor\DI\Definition\Exception\InvalidAnnotation;
use InstagramFeed\Vendor\DI\Definition\ObjectDefinition;
use InstagramFeed\Vendor\DI\Definition\ObjectDefinition\MethodInjection;
use InstagramFeed\Vendor\DI\Definition\ObjectDefinition\PropertyInjection;
use InstagramFeed\Vendor\DI\Definition\Reference;
use InstagramFeed\Vendor\Doctrine\Common\Annotations\AnnotationRegistry;
use InstagramFeed\Vendor\Doctrine\Common\Annotations\Reader;
use InstagramFeed\Vendor\Doctrine\Common\Annotations\SimpleAnnotationReader;
use InvalidArgumentException;
use InstagramFeed\Vendor\PhpDocReader\PhpDocReader;
use ReflectionClass;
...
class AnnotationBasedAutowiring implements DefinitionSource, Autowiring
{
    ...
    private function readProperty(ReflectionProperty $property, ObjectDefinition $definition, $classname = null)
    {
        ...
        $annotation = $this->getAnnotationReader()->getPropertyAnnotation($property, 'InstagramFeed\Vendor\DI\Annotation\Inject');
    ...
    private function getMethodInjection(ReflectionMethod $method)
    {
        // Look for @Inject annotation
        try {
            $annotation = $this->getAnnotationReader()->getMethodAnnotation($method, 'InstagramFeed\Vendor\DI\Annotation\Inject');
    ...
    public function getAnnotationReader()
    {
        if ($this->annotationReader === null) {
            ...
            $this->annotationReader->addNamespace('InstagramFeed\Vendor\DI\Annotation');
        }
    ...
    private function readInjectableAnnotation(ReflectionClass $class, ObjectDefinition $definition)
    {
        try {
            ...
            $annotation = $this->getAnnotationReader()->getClassAnnotation($class, 'InstagramFeed\Vendor\DI\Annotation\Injectable');
