...
class SBI_Global_Settings
{
	 ...
	 * @var string
	 */
	protected const SLUG = 'sbi-settings';
	...
	public function init()
	{
		...
		add_action('in_admin_header', array($this, 'maybe_remove_admin_footer'));
	...
	public function sbi_save_settings()
	{
		...
		// Handle license key updates.
		$this->handleLicenseKeyUpdates($data);
	 ...
	 * @return void
	 */
	private function handleLicenseKeyUpdates($data)
	{
		$sbi_license_key = sanitize_text_field($data['sbi_license_key']);
		...
		if (get_option('sbi_license_status') === 'inactive') {
			...
			if (empty($sbi_license_key) && !empty($license_key)) {
				$this->get_license_data($license_key, 'deactivate_license', SBI_PLUGIN_NAME);
	 ...
	 * @param string $license_action License action.
	 * @param string $item_name Item name.
	 *
	 * @return void|array $sbi_license_data License data.
	 * @since 6.0
	 */
	public function get_license_data($license_key, $license_action = 'check_license', $item_name = SBI_PLUGIN_NAME)
	{
		...
		return (array)SBI_HTTP_Request::data($request);
	}
	 ...
	 *
	 * @param array $sbi_license_data License data.
	 *
	...
	public function get_license_error_message($sbi_license_data)
	{
		...
		$upgrade_url = sprintf('https://smashballoon.com/instagram-feed/pricing/?license_key=%s&upgrade=true&utm_campaign=instagram-pro&utm_source=settings&utm_medium=upgrade-license', sanitize_key($license_key));
		$renew_url = sprintf('https://smashballoon.com/checkout/?license_key=%s&download_id=%s&utm_campaign=instagram-pro&utm_source=settings&utm_medium=upgrade-license&utm_content=renew-license', sanitize_key($license_key), sanitize_key($sbi_download_id));
	 ...
	 * Update License Data
	 *
	 * @param array  $license_data License data from the API.
	 * @param string $item_name Item name.
	 * @param string $option_name Option name.
	 *
	 * @return bool $license_changed True if license status changed.
	 * @since 6.0
	 */
	...
	public function maybe_remove_admin_footer()
	{
		if (Util::isIFPage()) {
			...
			add_filter('update_footer', array($this, 'remove_admin_footer_text'), 11);
		}
	}
	...
	public function builder_enqueue_admin_scripts()
	{
		...
		$sbi_settings = $this->getGlobalSettingsData();
	...
	/**
	 * Retrieves all the Global Settings data to localize.
	 *
	 * @return array
	 */
	public function getGlobalSettingsData()
	{
		global $wp_version;

		$model = $this->get_settings_data();
		$license_data = $this->getLicenseDataInfo();
		...
		return array(
			...
			'licenseStatus' => $license_data['status'],
			'licenseErrorMsg' => $license_data['errorMsg'],
			'hasError' => $license_data['hasError'],
			'upgradeUrl' => $upgrade_url,
			'model' => $model,
			'feeds' => $this->getExportedFeeds(),
			'sources' => SBI_Feed_Builder::get_source_list(),
			...
			'generalTab' => $this->getGeneralTabData($upgrade_url),
			'feedsTab' => $this->getFeedsTabData(),
			'advancedTab' => $this->getAdvancedTabData(),
			'codeSnippetsTab' => $this->getSnippetsTabData(),
	...
	/**
	 * Get license data information.
	 *
	 * @return array
	 */
	private function getLicenseDataInfo()
	{
		$license_key = trim(get_option('sbi_license_key'));
		$sbi_license_data = ['status' => 'inactive', 'errorMsg' => null, 'hasError' => false];
		...
		if ($license_key) {
			...
			$sbi_license_data = [
				'status' => $sbi_license_data['license'],
				'errorMsg' => $sbi_license_data['errorMsg'] ?? null,
				'hasError' => (isset($sbi_license_data['license']) && 'expired' === $sbi_license_data['license']) ||
	 ...
	 * @return array
	 */
	public function getExportedFeeds()
	{
		$exported_feeds = SBI_Db::feeds_query(['all_feeds' => true]);
		$feeds = array();

		foreach ($exported_feeds as $feed) {
			$feeds[] = array(
				'id' => $feed['id'],
				'name' => $feed['feed_name'],
			);
	...
	/**
	 * Get data for the General Tab
	 *
	 * @param string $upgrade_url The url to upgrade pro tiers.
	 * @return array[]
	 */
	public function getGeneralTabData($upgrade_url)
	{
		return array(
			'licenseBox' => array(
				...
				'upgradeText1' => sprintf(__('You are using the Lite version of the plugin–no license needed. Enjoy! 🙂 To unlock more features, consider <a href="%s">Upgrading to Pro</a>.', 'instagram-feed'), $upgrade_url),
		...
		return array(
			...
			'gdprBox' => array(
				...
				'tooltip' => '<p><strong>If set to “Yes”,</strong> it prevents all images and videos from being loaded directly from Instagram’s servers (CDN) to prevent any requests to external websites in your browser. To accommodate this, some features of your plugin will be disabled or limited. </p>
                    ...
                    <p><a href="https://smashballoon.com/doc/instagram-feed-gdpr-compliance/?instagram&utm_campaign=instagram-pro&utm_source=settings&utm_medium=docs" target="_blank" rel="noopener">Learn More</a></p>',
	...
	{
		$gdpr_doc_url = 'https://smashballoon.com/doc/instagram-feed-gdpr-compliance/?instagram&utm_campaign=instagram-pro&utm_source=settings&utm_medium=docs';
		$output = '';
		$active_gdpr_plugin = SB_Instagram_GDPR_Integrations::gdpr_plugins_active();
		if ($active_gdpr_plugin) {
			...
			$output = sprintf(
				/* translators: %s: placeholder for the formatted string */
				__('No GDPR consent plugin detected. Install a compatible <a href="%s" target="_blank">GDPR consent plugin</a>, or manually enable the setting to display a GDPR compliant version of the feed to all visitors.', 'instagram-feed'),
				$gdpr_doc_url
			);
		}
		return $output;
	...
	/**
	 * Get data for the Advanced Tab
	 *
	 * @return array[]
	 */
	public function getAdvancedTabData()
	{
		$usage_tracking_url = 'https://smashballoon.com/instagram-feed/usage-tracking/?utm_campaign=instagram-pro&utm_source=settings&utm_medium=docs';
		$feed_issue_email_url = 'https://smashballoon.com/email-report-is-not-in-my-inbox/?instagram&utm_campaign=instagram-pro&utm_source=settings&utm_medium=docs';
		...
		return array(
			...
			'usageBox' => array(
				...
				'helpText' => sprintf(
					// translators: %s is a "Learn More" link to the usage tracking documentation.
					__( 'Send a weekly report to Smash Balloon to help improve the product. No sensitive data is collected. You can disable this at any time. %s', 'instagram-feed' ),
					'<a href="' . $usage_tracking_url . '" target="_blank">' . __( 'Learn More', 'instagram-feed' ) . '</a>'
				),
			),
			'resetErrorBox' => array(
			...
			'feedIssueBox' => array(
				...
				'helpText' => sprintf(__('If the feed is down due to a critical issue, we will switch to a cached version and notify you based on these settings. <a href="%s" target="_blank">View Documentation</a>', 'instagram-feed'), $feed_issue_email_url),
	...
	/**
	 * Get data for the Snippets tab.
	 *
	 * @return array
	 */
	public function getSnippetsTabData()
	{
		return array(
