...
== Changelog ==
= 6.11.4 =
* New: Added Divi 5 Visual Builder compatibility for feed previews.
...
* Tweak: Corrected typos and grammar in admin labels, tooltips, and notification messages.
* Fix: Setup wizard tooltips now display correctly in the WordPress 7.0 iframed block editor.
* Fix: Elementor editor preview no longer renders masonry/highlight feeds with overlapping posts in Chromium-based browsers.
* Fix: Gutenberg blocks no longer break when multiple Smash Balloon plugins are active at the same time.
...
* Fix: Cached Instagram images are now saved at the intended compression quality on WordPress 6.8 and above; previously the resize step reset quality back to WordPress defaults, which could make thumbnail files larger than the full-size file.
* Fix: The "Thumbnail (150x150)" image resolution now serves the cached 150x150 file instead of falling back to the 320x320 version.

= 6.10.2 =
* Fix: Dragging a slide in a multi-image post no longer advances to the next post.
* Fix: Reels now open the correct post link when a WPConsent-style cookie banner is active.
* Fix: Mixed feeds with moderation now load quickly, surface queued posts, hide the empty "Load More" state, and display reel images.
* Fix: Carousel markup now uses a valid ARIA role for screen-reader compatibility.
* Fix: Instagram Feed Pro no longer removes the WordPress footer on plugin admin pages.
* Fix: Instagram Trial Reels (test posts shown only to non-followers) are no longer included in feeds.

= 6.10.1 =
* Fix: Custom templates cache key now works correctly when enabled.
